...
// This version is modified to reliably detect multiple signals, even when some are very weak, 
// by preventing a strong signal's large tolerance zone from 'absorbing' distant, weak peaks.
...
let VALUE_THRESHOLD = 40;          // ADJUSTED: Lowered to 20 to capture weak signals
...
const CONSOLIDATION_TOLERANCE = 5; // Max indices between peaks to be considered one reflection (~1.1 cm)
...
const MAX_PLOT_DISTANCE_CM = 200; // FIXED: Plot ceiling (0 to 200 cm)
...
const PLOT_HEIGHT_LINES = 40;     // Doubled plot height for more resolution
const PLOT_HISTORY_FRAMES = 80; 
const HEADER_LINES = 4;
const SIGNAL_SUMMARY_LINES = 5;   // Lines reserved for signal details
...
function trackAndFilterSignals(samples) {
    ...
    // --- Aggressive Peak Consolidation (Group adjacent peaks into one dominant peak) ---
    ...
    for (const peak of sortedPotentialPeaks) {
        if (!currentGroup) {
            currentGroup = { indices: [peak.index], peaks: [peak] };
    ...
    for (let i = 0; i < finalPeaks.length; i++) {
        ...
        for (const [id, signal] of persistentSignals.entries()) {
            ...
            // --- NEW LOGIC: Prevent strong signals from 'capturing' weak, distant peaks ---
            // If the incoming peak is weak (its peak value is low), we require it to be within
            // a tighter tolerance to prevent a strong signal's large tolerance zone from
            // absorbing a separate weak signal.
            let effectiveTolerance = requiredTolerance;
            if (peak.peak < 40) { // Arbitrarily low peak value threshold
                // If peak is very weak, only match if it's very close (e.g., within 5 indices)
                effectiveTolerance = Math.min(requiredTolerance, BASE_DISTANCE_TOLERANCE + 5); 
            }
            // ------------------------------------------------------------------------------
            
            if (diff <= effectiveTolerance && diff < minDiff) {
                minDiff = diff;
                closestSignalId = id;
            }
        }
            ...
            if (frame) {
                ...
                for (const signalData of frame.allPersistentMedianDistances) {
                    ...
                    if (normalizedY === y && medianDist <= MAX_PLOT_DISTANCE_CM) {
                        ...
                        if (x === lastFrameIndex) {
                            // Use '>' for the newest frame to indicate flow direction
