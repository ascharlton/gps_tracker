...
const VALUE_THRESHOLD = 110;                 // 2. Min value for a sample to be considered a 'potential' peak (adjust to filter noise).
...
const CONSOLIDATION_TOLERANCE_INDICES = 5;  // Max indices between consistent points to group them into a single reflection.
let EMA_ALPHA = 0.1;                        // Smoothing factor for the main header distance (0.1 is moderate smoothing).

// --- DYNAMIC DEAD ZONE CONFIG ---
const NOISE_FLOOR_RANGE = 100;              // The number of tail samples to average for noise floor.
const BLIND_ZONE_SEARCH_LIMIT = 500;        // Max index to search for the end of the pulse ring-down.
const BLIND_ZONE_THRESHOLD_FACTOR = 1.2;    // Threshold for ending blind zone (1.2 * NoiseFloor).
const BLIND_ZONE_DEFAULT_END = 350;         // Fallback index if blind zone isn't found within search limit.
...
const MAX_PLOT_DISTANCE_CM = 250;           // Y-axis maximum distance (0 to 250 cm).
...
class SignalTracker {
    ...
    updateAndGetConsistentIndices(currentValues, deadZoneIndex) {
        ...
        // START index check AFTER the DYNAMIC deadZoneIndex
...
function drawFixedHeader(latencyUs, totalPersistentCount, smoothedDistance, finalSignals, noiseFloor) {
    ...
    const line3 = `Tracking Config: ${ANSI_BOLD}V=${VALUE_THRESHOLD}${ANSI_RESET} (Value), ${ANSI_BOLD}C=${CONSISTENCY_SAMPLES}${ANSI_RESET} (Frames), ${ANSI_BOLD}T=${POSITION_TOLERANCE}${ANSI_RESET} (Pos Tol), ${ANSI_BOLD}G=${CONSOLIDATION_TOLERANCE_INDICES}${ANSI_RESET} (Group Tol)`;
    const line4 = `Dynamic Dead Zone: ${ANSI_YELLOW}0-${dynamicDeadZoneIndex} indices (${deadZoneDistance.toFixed(1)} cm)${ANSI_RESET} | Noise Floor: ${noiseFloor.toFixed(2)}`;
    const line5 = `Plot (X-Y: 0-${MAX_PLOT_HISTORY_FRAMES} Frames / 0-${MAX_PLOT_DISTANCE_CM} cm): ${ANSI_CLEAR_LINE}`;
    ...
    process.stdout.write(line5.padEnd(TERMINAL_WIDTH) + ANSI_CLEAR_LINE + '\n');

    // Reflection Summary (Line 6+)
...
function drawScrollingPlot() {
    ...
    const plotStartLine = 8; // Header (5) + Summary (2) + Separator (1)
    ...
    for (let y = 0; y < PLOT_HEIGHT_LINES; y++) {
        ...
        for (let x = 0; x < plotWidth; x++) {
            ...
            let currentFrameDeadZoneIndex = frame ? frame.deadZoneIndex : dynamicDeadZoneIndex;
            
            // Calculate dead zone row for the current frame
            const deadZoneRow = Math.floor((MAX_PLOT_DISTANCE_CM - (currentFrameDeadZoneIndex * SAMPLE_RESOLUTION)) / MAX_PLOT_DISTANCE_CM * PLOT_HEIGHT_LINES);
...
function updateConsoleDashboard(latencyUs, finalSignals, noiseFloor) {
    ...
    drawFixedHeader(latencyUs, totalPersistentCount, smoothedDistance, finalSignals, noiseFloor);

    // 3. Draw plot (starts at y=8)
    const plotStartLine = 8;
...
function processDataPacket(rawSamples) {
    ...
    // --- 1. DYNAMIC DEAD ZONE CALCULATION ---
    const noiseFloor = calculateNoiseFloor(rawSamples);
    const deadZoneIndex = findBlindZoneEnd(rawSamples, noiseFloor);
    dynamicDeadZoneIndex = deadZoneIndex; // Update global state for header/plot

    // --- 2. FIND CONSISTENT INDICES ---
    // The dynamic dead zone index is passed here to filter out the ring-down noise.
    const consistentIndices = signalTracker.updateAndGetConsistentIndices(rawSamples, deadZoneIndex);
    ...
    updateConsoleDashboard(latencyUs, finalSignals, noiseFloor);
}
