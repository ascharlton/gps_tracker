...
const NOISE_FLOOR_RANGE = 200;          // 200 how far from the end of the signal to sample for noise
...
/**
 * Executes the pulse detection logic and returns an array of 7 signal indices.
 * @param {Array<number>} values - The raw sample values.
 * @param {Set<number>} consistentIndices - The indices of consistent peaks.
 * @returns {Array<number>} Array of 7 signal indices, or -1 if not found.
 */
function extractSignalIndices(values, consistentIndices) {
    const signalIndices = new Array(NUM_SIGNALS).fill(-1);
    ...
    for (let i = 0; i < values.length; i++) {
        ...
        if (consistentIndices.has(i) && values[i] >= SIGNAL_THRESHOLD) {
            ...
            signalIndices[signalCount] = currentSignalStart;
            lastPulseEnd = currentPulseEnd;
            signalCount++;
            
            // Break if we've found all 7
    ...
    try {
        ...
        port.on('data', data => {
            ...
            while ((packet = extractAndProcessPacket())) {
                ...
                const signalIndices = extractSignalIndices(values, consistentIndices);
                
                // Filter signals: only print signals starting AFTER the blind zone
                const currentFrameSignals = signalIndices.map(idx => 
                    (idx !== -1 && idx >= blindZoneIndexEnd) ? idx : -1
                );
                
                // Add to history and maintain size
                plotHistory.push(currentFrameSignals);
                ...
                console.log(`Detected Signals (Start Index): ${currentFrameSignals.filter(i => i !== -1).join(', ') || 'None'}`);
