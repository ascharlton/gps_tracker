...
const CONSISTENCY_SAMPLES = 9;         // >5 becuase noise appears every second Minimum number of samples a peak must appear in to be 'consistent'
...
const SIGNAL_SEPARATION_THRESHOLD = 5; 

const MAX_BLIND_ZONE_SEARCH_SAMPLES = 200; 
...
/**
 * Calculates a dynamic blind zone end index using dual criteria:
 * 1. Noise Floor Crossing (Original Logic): Caters for clean ring-down where amplitude stabilizes quickly.
 * 2. Shape Pattern Detection (New Logic): Caters for scenarios where a close signal merges with the ring-down, 
 * using a decrease-then-rise pattern to identify the target's start.
 * * @param {Array<number>} values The raw sample values.
 * @param {number} noiseFloor The calculated noise floor average.
 * @returns {number} The earliest index where the blind zone is considered finished.
 */
function findBlindZoneEnd(values, noiseFloor) {
    // --- 1. Noise Floor Criteria (Original Logic) ---
    const NOISE_MARGIN_FACTOR = 1.2; 
    const NOISE_CONSISTENCY_COUNT = 5; 
    const noiseThreshold = noiseFloor * NOISE_MARGIN_FACTOR;
    let consecutiveNoiseCrossCount = 0; 

    // --- 2. Shape Pattern Criteria (New Logic) ---
    const MIN_START_INDEX = 100;    // 100 Don't look for the pattern before this index
    const MIN_TREND_COUNT = 5;      // 5 Required length for continuous decrease/rise
    let decreaseCount = 0;
    let increaseCount = 0;
    let sufficientDecreaseOccurred = false;

    // Start search from index 1 (to allow comparison with [i-1]) up to the max search limit
    for (let i = 1; i < MAX_BLIND_ZONE_SEARCH_SAMPLES; i++) {
        const currentVal = values[i];
        const prevVal = values[i - 1];
        
        // =======================================================
        // A. CHECK NOISE FLOOR CRITERION (Termination Priority)
        // =======================================================
        if (currentVal < noiseThreshold) {
            consecutiveNoiseCrossCount++;
            if (consecutiveNoiseCrossCount >= NOISE_CONSISTENCY_COUNT) {
                // Criterion 1 Met: Return the start of the consistent low-value zone
                return i - NOISE_CONSISTENCY_COUNT; 
            }
        } else {
            consecutiveNoiseCrossCount = 0;
        }

        // =======================================================
        // B. CHECK SHAPE PATTERN CRITERION (Termination Priority)
        // =======================================================
        
        // Update trend counters based on strict change
        if (currentVal < prevVal) {
            // Trend is decreasing (Ring-down tail)
            decreaseCount++;
            increaseCount = 0; 
            
        } else if (currentVal > prevVal) {
            // Trend is increasing (Start of a close reflection)
            
            // Check if the required decrease streak (Criterion 2) was just completed
            if (decreaseCount >= MIN_TREND_COUNT) {
                sufficientDecreaseOccurred = true;
            }
            
            increaseCount++;
            decreaseCount = 0; 

            // Check final criteria for shape-based end
            if (i >= MIN_START_INDEX && sufficientDecreaseOccurred && increaseCount >= MIN_TREND_COUNT) {
                // Criterion 2 Met: Return i - 3 (the low-point index before the 3-sample rise began).
                // This is a more definitive sign of a close target.
                return i - MIN_TREND_COUNT;
            }

        } else {
            // Values are equal (Current == Previous). Trends are unaffected.
        }
    }

    // If neither pattern is found within the search area, default to the maximum search area end
...
function extractAndProcessPacket() {
    ...
    try {
        ...
        // 4. Validate Blind Zone Amplitude in case we need to drop the packet
    ...
    try {
        ...
        port.on('data', data => {
            ...
            while ((packet = extractAndProcessPacket())) {
                ...
                // I think this is causing a bug as it is separate from any other logic - each algorithm is running independantly of each other
